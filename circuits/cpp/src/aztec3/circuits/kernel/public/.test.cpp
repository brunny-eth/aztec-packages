                                             ...
                                             NT::uint32 seed = 0)
{
    ...
    std::array<NT::fr, MAX_PUBLIC_CALL_STACK_LENGTH_PER_CALL> public_call_stack =
        array_of_values<MAX_PUBLIC_CALL_STACK_LENGTH_PER_CALL>(count);
    std::array<NT::fr, MAX_NEW_COMMITMENTS_PER_CALL> new_commitments =
        array_of_values<MAX_NEW_COMMITMENTS_PER_CALL>(count);
    std::array<NT::fr, MAX_NEW_NULLIFIERS_PER_CALL> new_nullifiers =
        array_of_values<MAX_NEW_NULLIFIERS_PER_CALL>(count);
    std::array<NT::fr, MAX_NEW_L2_TO_L1_MSGS_PER_CALL> new_l2_to_l1_msgs =
        array_of_values<MAX_NEW_L2_TO_L1_MSGS_PER_CALL>(count);
    std::array<ContractStorageRead<NT>, MAX_PUBLIC_DATA_READS_PER_CALL> reads = generate_contract_storage_reads(count);
    ...
    auto public_circuit_public_inputs = PublicCircuitPublicInputs<NT>{
    ...
    NT::fr const& contract_address)
{
    ...
    for (size_t i = 0; i < MAX_PUBLIC_DATA_UPDATE_REQUESTS_PER_TX; i++) {
        auto& update_request = update_requests[i];
...
PublicCallData<NT> get_public_call_data(NT::uint32& seed)
{
    ...
    NT::fr portal_contract_address = 23456;

    NT::address msg_sender = NT::fr(1);
    ...
    //***************************************************************************
    auto tx_request = TxRequest<NT>{ .origin = contract_address,
                                     .function_data = function_data,
    ...
    fr unencrypted_log_preimages_length = ++seed;
